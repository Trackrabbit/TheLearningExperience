...
SET ANSI_NULLS ON
GO
--exec DE_VoucherCreateCustomer 
--grant select on de_SchoolGpSiteXref to public
CREATE proc [dbo].[DE_VoucherCreateCustomer]
...
--FROM (
	...
	SELECT distinct
		--V.[id]
		'CR-'+MASCU.CUSTID+'-2012' AS [customer number]
		...
		--Left(case 
		--		when  tle_franchiseeidname IS NULL	then upper('CR-'+left(replace(replace( cr.tle_name,' ' ,''),',',''),6)) 
		--		when  NOT (tle_franchiseeidname IS NULL) AND EXISTS (SELECT left(replace(replace(UPPER(ci.tle_name),' ' ,''),',',''),7) as center 
		--													FROM [TLEFLSQL8.TLECORP.COM].tleco.dbo.gpcenter ci
		--													WHERE NOT (ci.tle_franchiseeidname IS NULL) and left(replace(replace(UPPER(ci.tle_name),' ' ,''),',',''),7) = left(replace(replace(UPPER(cr.tle_name),' ' ,''),',',''),7)
		--													GROUP BY left(replace(replace(UPPER(ci.tle_name),' ' ,''),',',''),7),ci.statecodename,ci.statuscodename
		--													HAVING COUNT(*) > 1 AND ci.statecodename = 'Active' and ci.statuscodename = 'Open')
		--		THEN upper('CR-'+replace(replace( cr.tle_franchiseeidname,' ',''),',','')) 
		--		else upper('CR-'+replace(replace( cr.tle_name,' ',''),',','')) end,10)+'-2012' as [customer number]
		,s.school_id
		--,s.period
		--,s.year
		--,s.submitted
		...
		--,[FoodCost]
		--,[WorkingCapital],
		,cr.tle_CenterTypeName as [School Type],mascu.*,CR.tle_gpsite
	FROM 
		...
		join [TLEFLSQL1].[mas500_app].[dbo].[tarCustomer] mascu on mascu.custkey=s.school_id
	...
	where 
		...
		and [Customer Number] not in (Select CUSTNMBR from RM00101 where left(CUSTNMBR,2)='CR')
		--and [year] = '2015'
		--and period = '11'
--and 
