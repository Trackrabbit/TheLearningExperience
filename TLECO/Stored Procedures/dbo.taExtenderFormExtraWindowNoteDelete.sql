...
SET ANSI_NULLS ON
GO
create procedure [dbo].[taExtenderFormExtraWindowNoteDelete]  
	@I_vFormID  char(15),    
	@I_vRecordID  char(15),
...
if @I_vFormID = '' or @I_vRecordID = '' or @I_vWindowNumber = 0 or @I_vNoteNumber = 0
	begin
		select @O_iErrorState = 25100
			...
			if dbo.ExtenderDoesFormExist(@I_vFormID) = 0
				...
				begin
					...
					if (select count(*) from EXT20201 where Extender_Form_ID = @I_vFormID and Window_Number = @I_vWindowNumber and (Extra_Window_Type = 2 or Extra_Window_Type = 3)) = 0
						begin
							select @O_iErrorState = 25083
							exec @iStatus = taUpdateString @O_iErrorState, @oErrString, @oErrString output, @iAddCodeErrState output
						end
					else
						begin
							exec ExtenderFormGetRecordID @I_vFormID, @I_vRecordID, @I_vRecordID2, @RecordID output
							delete EXT01500 where Extender_Record_ID = @RecordID and Extender_Type = 2 and LNITMSEQ = @I_vNoteNumber
						end
				end
		end
	end
...
GO
GRANT EXECUTE ON  [dbo].[taExtenderFormExtraWindowNoteDelete] TO [DYNGRP]
GO
