...
Create proc [dbo].[pGetAnnualScheduleLease] @leaseId char(15)
as
...
[tle_Leasenum]
      ,[Year1] as Year1
      ,Max(Schedulename) Schedulename
	  , MIN(baserentamount)  as Rentsqfoot
	  ,MIN(baserentamount)*10   annualamaount,
      min([monthlyamount]*10) monthlyamount
	,case when Row_Number() over (order by tle_Leasenum)=1 then 0 else round(MIN(Convert(numeric(19,5),[Increase])),2) end as Increase
  into ##templease5
FROM [dbo].[DE_GroundLease_LeaseReport_Detail_vw]
where 
tle_Leasenum = @leaseId
and 
Schedulename like '1M%'
  group by [tle_Leasenum],[Year1]
  order by [tle_Leasenum],[Year1]
  option (maxrecursion 300)
  --select * from [DE_GroundLease_LeaseReport_Detail_vw]
  insert into ##templease5 
  select tle_Leasenum,Year1,Schedulename,NewRentAmount as Rentsqfoot, NewRentAmount*SquareFoot as AnnualAmaount,
  monthlyamount,round(Increase*10000,2) as Increase
  --into ##templease5 
 from ##TMPLease where Year1 between 2 and 15--order by tle_LeaseNum,tle_termstartd asc
  insert into ##templease5 
  select 'Option #1',0,'',0,0,0,0
   insert into ##templease5 
  select tle_Leasenum,Year1,Schedulename,NewRentAmount as Rentsqfoot, NewRentAmount*SquareFoot as AnnualAmaount,
  monthlyamount,round(Increase*10000,2) as Increase
  --into ##templease5 
 from ##TMPLease where Year1 between 16 and 20
   insert into ##templease5 
  select 'Option #2',0,'',0,0,0,0
     insert into ##templease5 
  select tle_Leasenum,Year1,Schedulename,NewRentAmount as Rentsqfoot, NewRentAmount*SquareFoot as AnnualAmaount,
  monthlyamount,round(Increase*10000,2) as Increase
  --into ##templease5 
 from ##TMPLease where Year1 between 21 and 25
 update ##templease5 set annualamaount=b.annualamaount,monthlyamount=b.monthlyamount
 from
 (
 select * from ##templease5 where Year1=2
 ) b
 where ##templease5.Year1=1 and ##templease5.tle_Leasenum=b.tle_Leasenum 
 --and ##templease5.Schedulename = b.Schedulename




select tle_Leasenum,Year1,Schedulename,Rentsqfoot ,annualamaount,monthlyamount,convert(int,increase) increase from ##templease5
...
DROP TABLE ##TMPLease4
DROP TABLE ##templease5
GO
GRANT EXECUTE ON  [dbo].[pGetAnnualScheduleLease] TO [DYNGRP]
GO
