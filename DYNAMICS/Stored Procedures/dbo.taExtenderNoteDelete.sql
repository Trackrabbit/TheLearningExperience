	...
	begin
		...
		begin
			...
			begin
				...
				begin
					set @FOUND = 1
					set @CMD = 'exec ' + rtrim(@INTERID) + '.dbo.taExtenderNoteDelete @winID, @key1, @key2, @key3, @key4, @key5, @noteNo, @state output, @error output '
					exec sp_executesql @CMD, N'@winID char(15), @key1 char(255), @key2 char(255), @key3 char(255), @key4 char(255), @key5 char(255), 
								@noteNo int, @state int output, @error char(255) output',
						@I_vWindowID, @I_vKey1, @I_vKey2, @I_vKey3, @I_vKey4, @I_vKey5, @I_vNoteNumber, @O_iErrorState output, @oErrString output
				end
			end
			fetch next from company_cursor into @INTERID
		end
